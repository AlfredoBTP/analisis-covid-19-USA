...
st.title("Recomendaciones para un manejo mas eficaz")
muertes =df['deaths_covid'].sum()
st.markdown(f'Al dia de hoy estados unidos tiene un total de {muertes}')
st.markdown('Lo que nos hace reflecionar que es lo que podemos hacer para evitar que este número de muertes sea menor')

st.markdown('Si miramos cada uno de los gráficos podemos observar 2 cosas que fueron claves para reducir el número de muertes, la primera de ellas es el confinamiento y la segunda es la vacunación.')
st.markdown('Tambien podemos notar que la disponibilidad de camas UCI y la falta de personal, influyen en el aumento de la tasa de mortalidad')
st.markdown('Así que las recomendaciones serían las siguientes:')
st.markdown('Alargar la cuarentena lo más posbile, en especial a los sectres vulnerables hasta no contar con un tratamiento eficaz o un programa de vacunación')
st.markdown('Mantener las medidas recomendadas por la OMS a pesar de que ya inicio la vacunación')
st.markdown('Evitar realizar eventos masivos sin exigir el carnet de vacunación mientras que se encuentre una cantidad de casos activos considderables')
df_completo= pd.read_csv(r"C:\Users\Alfredo BTP\Henry Data Science\PI dos\data\pages\df.csv")
...
df_completo['Total hospitalizados'] = df_completo['Total_confirmados_pediatricos'
            ]+ df_completo['Total_confirmados_adultos'] + df_completo['Total_confirmados_UCI']
...
df_completo['Muertes por covid-19'].fillna(0,inplace=True)
...
enfermos_tot.index= pd.to_datetime(muertes_tot.index.values)
enfermos_tot['Año'] =enfermos_tot.index.year
#muertes_tot['Año'] =muertes_tot['Año'].astype(str)
lista_años =  enfermos_tot['Año'].unique().tolist()
año =st.selectbox('Selecciona un año', lista_años,0)
enfermos_tot = enfermos_tot[enfermos_tot['Año'] == año]   
...
muertes_tot.index= pd.to_datetime(muertes_tot.index.values)
muertes_tot['Año'] =muertes_tot.index.year
muertes_tot = muertes_tot[muertes_tot['Año'] == año]
